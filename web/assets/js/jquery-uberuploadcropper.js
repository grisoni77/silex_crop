	...
	$.fn.uberuploadcropper = function(options){
		...
		var allUploadsComplete = function(){
			...
			$.each(imgdata,function(i,obj){
				var btn = {};
				var str = '<div style="text-align: center;"><img src="'+ options.folder + obj.uploadName +'?d='+ (new Date()).getTime() +'" class="ubercropimage" id="ubercropimage_'+ i +'" style="margin: 0 auto;" /></div>'+
					'<input type="hidden" name="imgcrop['+ i +'][filename]" id="imgcrop_'+ i +'_filename" value="'+obj.uploadName+'" class="imgcrop_filename" />'+
					'<input type="hidden" name="imgcrop['+ i +'][originalFilename]" id="imgcrop_'+ i +'_originalFilename" value="'+obj.uploadName+'" class="imgcrop_originalFilename" />'+
			...
			.on('complete', function(event, id, fileName, responseJson){
					...
					if ($t.fineUploader('getInProgress') == 0) {
						...
						$t.trigger('uberAllUploadsComplete',[imgdata]);
                         ...
                         .on('validate', options.fineuploader.callbacks.onValidate) 
                         ;
